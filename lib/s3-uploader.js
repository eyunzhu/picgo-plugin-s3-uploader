...
function normalizeUserConfig(userConfig) {
  ...
  return {
    ...
    contentDispositionInline: normalizeBoolean(
      safeUserConfig.contentDispositionInline,
...
async function uploadObjectToS3({ userConfig, key, payload, createDate, localeLike }) {
  ...
  if (userConfig.contentDispositionInline) {
...
function createPluginConfig(ctx) {
  ...
  return [
    ...
    {
      name: 'contentDispositionInline',
      type: 'confirm',
      default: userConfig.contentDispositionInline,
      required: false,
      alias: translateMessage(locale, 'cfg_content_disposition_inline_alias'),
      message: translateMessage(locale, 'cfg_content_disposition_inline_message')
    }
  ]
}
